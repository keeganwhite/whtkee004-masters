...
echo "Created labels.csv with header row. These labels will include, the number of packets and the file name that the
flow is stored in."
...
do
     ...
     # 3) Append filename-label_stats pair to labels1.csv
     echo "${f}, ${flow_stats}" >> "$LABELS_FILE_CURRENT"
done
...
rm $NDPI_OUT_FILE
sudo chmod -R 777 "$LABELS_FILE_CURRENT"