...
def main():
    ...
    f = open("random_search_1d_144_3_300.csv", "w")
    model = KerasClassifier(build_fn=create_model)
    batch_size = [500, 750, 1000, 2000]
    epochs = [50, 75, 100, 150, 200]
    optimizers = ['SGD', 'RMSprop', 'Adam']
    filter_sizes = [14, 72, 144]
    kernel_sizes = [1, 3]
    cnn_activations = [LeakyReLU(alpha=0.1), 'relu']
    output_activations = ['softmax', 'sigmoid', 'relu']
    ...
    grid_cnn = RandomizedSearchCV(model, param_grid_cnn, cv=KFold(n_splits=num_folds, shuffle=True), verbose=0,
                                  n_iter=500,
                                  scoring=make_scorer(f1_score, average='macro'), random_state=1)
    X = x_k_fold
